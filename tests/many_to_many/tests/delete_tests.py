...
class DeleteManyToManyTest(TestCase):
    def setUp(self):
        set_up_helper()
    ...
    def test_delete_m2m_not_affect_other_category(self):
        m2m = ProductCategory.objects.filter(position=0, category_id=1).first()
        m2m.delete()
    ...
    def test_delete_product(self):
        item = Product.objects.filter(name="Cap").first()
        item.delete()
    ...
    def test_delete_product_also_removed_from_other_category(self):
        item = Product.objects.filter(name="Cap").first()
        item.delete()
