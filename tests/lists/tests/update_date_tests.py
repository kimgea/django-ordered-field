...
class ListUpdateDateTest(TestCase):
    ...
    def setUp(self):
        set_up_helper()
        self.pre_result = list(Item.objects.order_by("list", "id").
                          values_list("id", "created", "updated", "list", "order"))
        time.sleep(0.1)

    def _assert_helper(self, changed):
        post_result = list(Item.objects.order_by("list", "id").
                           values_list("id", "created", "updated", "list", "order"))
        ...
        for i in range(len(post_result)): # Feels bad to check in loop
            if (post_result[i][3], post_result[i][4]) in changed:
                self.assertNotEqual((self.pre_result[i][0], self.pre_result[i][2]),
                                (post_result[i][0], post_result[i][2]))
            else:
                self.assertEqual((self.pre_result[i][0], self.pre_result[i][2]),
                                    (post_result[i][0], post_result[i][2]))
